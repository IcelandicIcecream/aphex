...
4. **Push to standalone repo** — when changes to `templates/base/**` land on `main`, `.github/workflows/sync-template.yml` mirrors the folder to `IcelandicIcecream/aphex-base`, rewriting `workspace:*` deps to real versions before pushing.

   **Mirroring is not shipping.** `create-aphex` fetches that repo via `giget` at a pinned tag, not at `main`: `options.ts` builds the ref `create-aphex-v${packageVersion.version}` from its own version. The workflow cuts that tag on the mirror commit it creates, and **refuses to move a tag that already exists** (`sync-template.yml:218` — "main may advance but the snapshot will not move"). So a template-only change reaches the mirror's `main` and no user at all until `create-aphex` is released at a new version, which is what cuts a tag pointing at it.

   Any template fix therefore needs a `'create-aphex': patch` changeset. It cannot be a changeset on `@aphexcms/base` or `@aphexcms/website` — both are in `ignore` in `.changeset/config.json`, so changesets rejects them. Bumping a package whose own source did not change is intentional here; see `b3b1060b` and `324b56f6` for the same move done as a hotfix.
