...
New SQLite database adapter built on libsql: local `file:` databases (zero infra — perfect for blogs and single-user sites) and Turso-hosted `libsql://` URLs with the same adapter. Implements the full `DatabaseAdapter` interface with the same explicit organization-scoped WHERE filtering the PostgreSQL adapter uses; document JSON filters/sorts run through SQLite's `json_extract`.
