...
**Concurrency (compare-and-swap):** Every document write is guarded by a monotonic `revision` column on `cms_documents`, incremented on every draft write. A caller that read the document at revision N can pass `expectedRevision: N` to `updateDocDraft`/`publishDoc`/`unpublishDoc` (and `VersionService.restoreVersion`); a mismatch throws `RevisionConflictError` instead of silently overwriting a change made in between (a second tab, an AI agent, a concurrent request) — `expectedRevision` is optional everywhere, so omitting it preserves unconditional last-write-wins. Threaded end-to-end: adapter → `CollectionAPI` → zod request schemas (`expectedRevision` in, `revision` out via `_meta`) → HTTP routes (409 on conflict) → client (`api/documents.ts`) → `DocumentEditor.svelte`, which sends the revision it last read on every write and surfaces a 409 as "changed elsewhere, reload" rather than a generic error. **Non-obvious**: both `PostgreSQLAdapter`/`SQLiteAdapter` (`packages/{postgresql,sqlite}-adapter/src/index.ts`) wrap the document adapter with org-hierarchy fallback (retry against a child org if not found in the caller's org) — that wrapper must forward `expectedRevision` too, or CAS is silently a no-op end-to-end even though the inner adapter enforces it correctly. Proven cross-dialect by the `revision compare-and-swap` block in `packages/sqlite-adapter/tests/conformance.spec.ts` (run against both pglite and libsql). This is Milestone 1 of `references/content-copilot-phase-1-plan.md` — the concurrency-safety prerequisite for a future mutating content-copilot agent.
...
**Event, Queue & Job System (the durable "react to facts" spine):** The leg the architecture reserved for reactions — the Codex rule is **hooks transform, events react** (a hook mutates input in the write path; a consumer reacts out of band). Four parts, all DB-backed and organization-scoped, defined as ports in `packages/cms-core/src/lib/db/interfaces/events.ts` and implemented by the relational adapters:
...
**Recurring work is a chain, not a cron row.** There is no recurring-job type: a repeating job reschedules _itself_, each tick's handler enqueuing the next before it returns (`scheduleNextTick(adapter, job, { intervalMs })`). A tick is an ordinary job, so it inherits leases, backoff, dead-lettering and the Activity view, and a chain switches itself off per environment by simply not rescheduling. Starting one is the part that's quiet to get wrong: use `ensureRecurringJob(adapter, { organizationId, type })`, which decides on **liveness** (`pending`/`leased`) and not an idempotency key. A keyed bootstrap looks right and fails later — the bootstrap job completes immediately, so every later arming call gets that finished row back and no-ops, meaning a chain that dies can never be revived while the arming call still reports success. Call `ensureRecurringJob` from any path that implies the feature is in use (a settings panel opening, a manual "sync now"); it never starts a second live chain and always revives a dead one. Relatedly, `scheduleJob`'s `idempotencyKey` returns the existing row _whatever its status_ — pass `resurrect: true` to re-arm a `failed`/`cancelled` one, but never on a hot read path, where a permanently broken job would be re-armed on every request (`requeueJob`, i.e. the Activity view's Retry, is the operator's route for that).
...
- `packages/cms-core/src/lib/jobs/` — `run-due-jobs.ts` (the pure claim→run→settle engine + backoff policy), `relay.ts` (outbox fan-out), `run-batch.ts` (`runJobsBatch` — the shared relay+run seam), `document-jobs.ts` (scheduled publish/unpublish handlers), `recurring.ts` (`ensureRecurringJob`/`scheduleNextTick` — the chain pattern below)
