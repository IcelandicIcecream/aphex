...
export const GET: RequestHandler = async ({ locals }) => {
	...
	// Idempotent: wipe existing blog content so re-running gives a clean set
	// instead of duplicates. (siteSettings is a singleton — updated, not deleted.)
	...
	const posts = [
		...
		},
		{
			...
			content: [
				...
				li('Custom types sit between blocks — images, callouts, code', 'number'),
				p(
	...
	if (settingsId) {
		await settingsColl.update(
			...
			settingsId,
			{
				...
				nav: [
					{ _key: key(), label: 'About', url: '/about', newTab: false },
					{ _key: key(), label: 'Colophon', url: '/colophon', newTab: false }
				],
				social: [
					{ _key: key(), label: 'Twitter', url: 'https://twitter.com/example' },
					{ _key: key(), label: 'GitHub', url: 'https://github.com/example' }
				]
			} as never,
			{ publish: true }
		);
