...
<TypeTable
	type={{
		...
		owner: {
			type: 'role',
			description:
				'Full access — every capability, plus the hardcoded ability to delete the organization. Not editable; see below.'
		}
	}}
/>
...
`owner` doesn't mean "a role that happens to start with everything" — it means _every
capability_, continuously. Two consequences:
...
Core capabilities autocomplete, and plugin-declared ones (`'forms.export'`) are accepted too — see [Plugins](/plugins#add-a-permission).
...
`BaseField.access` narrows reads and writes at the individual field level. Field access is evaluated _after_ schema access — if you can't read the document, you'll never reach the field check.
