...
export const { auth, service, provider } = createAphexAuth({
	...
	options: {
		...
		inviteOnly: env.AUTH_INVITE_ONLY === 'true'
	}
});
```
...
caching and the verification-email throttle), [`socialProviders`](#oauth), and
[`betterAuth`](#the-escape-hatch). All are optional.
...
| `inviteOnly`               | `false` | Restrict account creation to addresses holding a pending, unexpired invitation.                                                                        |
...
<Callout type="warn">
	...
	There's no bootstrap exception either — with an empty user table nobody can sign up and nobody
	exists to invite, so **create the first admin before turning it on**.
</Callout>

## Schema

The auth tables (`user`, `session`, `account`, `verification`, `apikey`) ship per dialect. Your app
re-exports them from one local file:
...
Plugins like `organization` or `twoFactor` bring their own tables. Those aren't in the auth package —
which plugins you run is your choice — so define their tables in your own schema file and register
the plugin through [the escape hatch](#the-escape-hatch).
