...
Aphex uses a **capability-based** access control system. A user (or API key) carries a set of capability strings (`document.publish`, `member.invite`, `asset.upload`, …) and every protected operation is gated against one of them. Roles are named bundles of capabilities — and they're editable per organization.

For the full reference — including the capability list, custom roles, schema-level rules, field-level access, and the `/api/roles` endpoints — see [Access Control](/docs/access-control).

### Instance (system) roles

Every user has a system-wide role on their CMS profile, independent of any organization:

| Role          | Description                                                                                                             |
| ------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `super_admin` | First user to sign up. Every capability in every organization. Can never be locked out.                                 |
| `admin`       | Every capability in every organization. Intended for platform operators.                                                |
...
`super_admin` and `admin` at the **instance** level override everything that follows — they're the break-glass path.

### Organization roles

Each user also has a role **per organization** they belong to. Four built-in roles are seeded for every new organization:
...
Built-in roles are **editable** (you can add or remove capabilities). They can't be deleted. On top of these, each organization can define **custom roles** with any capability list. Schemas can then grant access to specific role names.

### Checking capabilities in code

Capabilities are resolved once per request by the auth hook (via `RolesService`) and cached on `auth.capabilities`, so checks are synchronous:
