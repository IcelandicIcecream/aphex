...
<Callout type="warn">
	...
	[bootstrap recipe](#bootstrapping-the-first-admin) — not by turning `inviteOnly` off.
</Callout>
...
Under the hood the form sets a short-lived `aphex_bootstrap_code` cookie just before sign-up. It has
to be a cookie rather than a header: the profile — and so the policy — runs on the first
authenticated request *after* sign-up, which is often a redirect the form never gets to decorate.

If you're driving sign-up yourself, the code is read from `?claim=<code>`, that cookie, or an
`x-aphex-bootstrap-code` header, in that order. Pass `readCode` to accept it from somewhere else:
