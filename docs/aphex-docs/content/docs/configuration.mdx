...
Controls document version history. Each publish creates an entry in the `cms_document_versions` table, accessible at `GET /api/documents/{id}/versions`.
...
| Option        | Type     | Default | Description                                                       |
| ------------- | -------- | ------- | ----------------------------------------------------------------- |
| `maxVersions` | `number` | `25`    | Max versions to keep per document (rolling). Set `0` to disable. |
