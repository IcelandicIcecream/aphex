...
`@aphexcms/sqlite-adapter` runs the same `DatabaseAdapter` contract on SQLite through [libsql](https://github.com/tursodatabase/libsql): a plain local `file:` database (no Docker, no server — ideal for blogs and single-user sites) or a Turso-hosted `libsql://` URL. The blog template uses it by default.
...
<Callout>
	...
	(breaks the per-transaction connections) and `query_only=ON` (the CMS writes) from
	read-optimized tuning guides.
</Callout>
