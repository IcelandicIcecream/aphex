...
<Step>
	**Run migrations.** `pnpm db:generate` locally → review the SQL → commit → run `pnpm db:migrate`
	against prod as a release step (not at boot — concurrent deploys will race).
</Step>
...
<Step>
	**Migration Pre-Deploy command:** set `pnpm db:migrate` so Render runs migrations before flipping
...
<Step>
	**On deploy** run `pnpm db:migrate` against production before serving traffic — Render's
	pre-deploy command, a Docker `exec` step, a CI job. Whatever your platform calls it.
</Step>
...
<Cards>
	...
	/>
	<Card title="Database" href="/database" description="Migrations workflow, RLS, multi-tenancy." />
	<Card
