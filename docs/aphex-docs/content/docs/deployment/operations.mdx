...
Auth endpoints are rate limited per client IP, which means the caller's address has to
survive the proxy in front of the app. Railway, Render, Fly and Coolify are detected
automatically. Anywhere else, name the header:

```bash
AUTH_IP_ADDRESS_HEADERS=x-forwarded-for   # nginx, Caddy, Traefik
AUTH_IP_ADDRESS_HEADERS=cf-connecting-ip  # behind Cloudflare
```

Until it resolves, the log warns and every caller shares one bucket per endpoint — so a
single password-reset request throttles everyone. Only name a header your proxy
overwrites: on a directly-reachable app it is client-supplied, and trusting it lets a
caller pick their own IP and skip rate limiting entirely.
