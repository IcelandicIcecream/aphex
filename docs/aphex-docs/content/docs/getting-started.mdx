...
```bash
npx aphx create
# or: pnpm dlx aphx create
```

Pick a lowercase-hyphen project name (e.g. `my-cms`). The [`aphx`](https://www.npmjs.com/package/aphx) CLI runs the scaffolder, which copies the **base template** into a new directory and rewrites workspace deps to published versions.
...
Open [http://localhost:5173/admin](http://localhost:5173/admin). The first account that signs up becomes the **super admin** and gets a default organization. Subsequent users sign up as `editor` and need to be invited.
...
`npx aphx create` copies `.env.example` to `.env`. For local dev the defaults just work — you only need to change things for production.
...
See [Configuration](/docs/configuration) for the full option reference.
...
See [Schema Types](/docs/schemas) for every field type, validation rules, references, arrays, and objects.
...
This writes to `src/lib/generated-types.ts` and augments `localAPI.collections` so queries are fully typed. Re-run it whenever you change a schema. See [Type Generation](/docs/type-generation) for details.
...
See [Storage](/docs/storage) for adapter options and provider examples.
...
<Cards>
	<Card
		...
		description="Every aphex.config.ts option — schemas, adapters, auth, GraphQL, cache, plugins."
	/>
	<Card
		title="Schemas"
		href="/docs/schemas"
