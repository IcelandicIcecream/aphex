...
This page stays the prose explanation of _how the API behaves_. The generated reference is the authoritative list of _what exists and what it takes_, because it is built at request time from the same sources the server runs on:
...
<Callout title="Generate a typed client">
...
`/api/openapi.json` requires authentication — it enumerates your entire content model, which is not something to hand to anonymous callers even though it carries no content. `/api/docs` is a static shell that fetches the spec from the browser with your session cookie, so the page itself exposes nothing.

The reference UI loads [Scalar](https://scalar.com) from a public CDN. On an instance that shouldn't pull third-party scripts, turn the page off — the JSON endpoint is unaffected:
...
Read-only observability over the [event, queue and job spine](/events-and-jobs), plus the two operator actions that make a dead letter recoverable. `?scope=all` widens a read from the active organization to the whole instance and is super-admin only.
...
<Callout type="warn" title="Two health endpoints, different answers">
    ...
    [deployment guide](/deployment) configures. Point your platform's health check at `/healthz`
...
The same operators back the Local API — see [filtering](/local-api#filtering). The canonical list lives in `packages/cms-core/src/lib/types/filters.ts`.

<Callout type="warn" title="Filters fail silently, in both directions">
    ...
    - **A misspelt operator matches everything.** `{ "slug": { "eq": "contact" } }` drops the clause
    entirely and returns every document of that type, with a `200`. A typo widens the result set
    instead of narrowing it.
    - **A malformed clause matches nothing.** A `where` of the wrong shape, or one naming a field
    your schema doesn't declare, returns `total: 0` — indistinguishable from an empty collection.
