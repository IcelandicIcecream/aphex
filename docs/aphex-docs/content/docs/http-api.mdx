...
The reference UI loads [Scalar](https://scalar.com) from a public CDN — the one thing it does that the JSON endpoint doesn't. On an instance that shouldn't pull third-party scripts, unmount it, after which it answers `404`:
