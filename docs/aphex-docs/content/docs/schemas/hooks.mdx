...
| **Reject** — required, format, cross-field invariants | [`validation`](/schemas/validation) (`Rule`) — never a hook |
...
`data` is typed **by self-reflection from the schema's own fields** — no generated types, no casts. This is why schemas are written with `defineType` rather than a `SchemaType` annotation: the function's generic parameter keeps the exact `fields` literal, and `data` is derived from it (see [why `defineType`](/schemas#why-definetype-and-not-a-type-annotation)). A schema declared as `const x: SchemaType = { ... }` erases its field literals, so its hooks get `data: Record<string, unknown>`.
...
Inferred field values are `T | undefined` because a write may carry only a subset of fields, so use optional access (`data.email?.…`). Field types map as you'd expect (`string`/`text`/`url`/`slug`/`date` → `string`, `number` → `number`, `boolean` → `boolean`, `reference` → `{ _type: 'reference'; _ref: string }`); richer fields fall back to `unknown` for now.
