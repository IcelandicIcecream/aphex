...
Aphex supports 12 built-in field types:
...
The `preview` property controls how documents and objects appear in lists:

```ts
preview: {
  select: {
    title: 'name',        // Field to use as the primary label
    subtitle: 'category', // Field to use as secondary text
    media: 'image'        // Field to use as thumbnail
  }
}
```
