...
- **Storage path:** `./storage/assets` (private — not inside SvelteKit's `static/`)
...
The base template configures its own local adapter in `src/lib/server/storage/index.ts` that falls back to **`./static/uploads`** (served by SvelteKit's static handler at `/uploads`) when no `R2_*` env vars are set. See [How the template chooses an adapter](#how-the-template-chooses-an-adapter) below.
...
## How the template chooses an adapter

The base template's `src/lib/server/storage/index.ts` picks an adapter at boot based on environment variables:
...
- **Dev** — no `R2_*` env vars set → writes to `static/uploads/`, served at `/uploads/...` by SvelteKit.
- **Prod** — fill in the `R2_*` env vars to use Cloudflare R2, AWS S3, MinIO, or any S3-compatible backend.

You can delete the conditional entirely if you know which adapter you want to use, or extend it to switch on other signals (e.g. a `STORAGE_ADAPTER=s3|local` env var).
