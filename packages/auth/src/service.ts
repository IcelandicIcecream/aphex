// @aphexcms/auth — AuthService.
//
// Centralises auth-related server operations: session → CMSUser assembly with
// organization resolution, API-key validation/CRUD, and user-profile helpers.
// `auth` (the hardened Better Auth instance) and `drizzleDb` are injected; the
// cms-core DatabaseAdapter is passed per call so the service stays decoupled.
...
import { user, apikey } from './schema/pg';
import type { AuthDrizzleDb } from './types';
import type { AphexAuth } from './instance';
...
/** Build the AuthService bound to a Better Auth instance + raw Drizzle handle. */
			...
			return userApiKeys.map((key) => {
				const metadata =
					typeof key.metadata === 'string'
						...
						: (key.metadata as Record<string, unknown> | null) || {};
				return {
					...key,
