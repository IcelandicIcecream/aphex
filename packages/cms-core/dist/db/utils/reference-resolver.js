 ...
 */
export async function resolveReferences(document, adapter, options) {
    ...
    if (document.draftData) {
        resolvedDocument.draftData = await resolveDataReferences(document.draftData, adapter, { depth, currentDepth: currentDepth + 1, visited });
    }
    // Resolve references in publishedData
    if (document.publishedData) {
        resolvedDocument.publishedData = await resolveDataReferences(document.publishedData, adapter, { depth, currentDepth: currentDepth + 1, visited });
    }
 ...
 */
async function resolveDataReferences(data, adapter, options) {
