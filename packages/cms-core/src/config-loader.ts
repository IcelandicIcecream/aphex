 ...
 */
export async function reloadConfig(): Promise<CMSConfig> {
  try {
    ...
    console.log('🔄 Config reloaded from fresh schemas:', {
      ...
      documents: schemaTypes.filter((type: any) => type.type === 'document').length,
      objects: schemaTypes.filter((type: any) => type.type === 'object').length
    });
