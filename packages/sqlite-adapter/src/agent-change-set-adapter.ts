...
/**
 * The agent audit/undo trail on SQLite. Unlike `SQLiteEventJobAdapter`, recording here is a
 * best-effort side observation (see the route handler that calls this), not atomic with the
 * document write's own transaction — so this class is never rebound onto a `withTransaction`
 * handle the way the event/job adapter is.
 */
export class SQLiteAgentChangeSetAdapter {
	constructor(
		private db: DB,
