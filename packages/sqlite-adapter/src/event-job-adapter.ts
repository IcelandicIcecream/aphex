...
export class SQLiteEventJobAdapter {
	...
	async appendEvent(input: AppendEventInput): Promise<DomainEvent> {
		...
		// Same insert path (same `this.db`, so same tx inside withTransaction): mirror the event
		// into the relay's worklist. `event_type`/`payload` denormalized for join-free fan-out.
