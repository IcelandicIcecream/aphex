// SQLite organization adapter implementation
 ...
 */
export class SQLiteOrganizationAdapter implements OrganizationAdapter {
	...
	async cleanupExpiredInvitations(): Promise<number> {
		// SQLite has no NOW() — compare against a JS Date (timestamp_ms column)
		const result = await this.db
			.delete(this.tables.invitations)
			.where(lt(this.tables.invitations.expiresAt, new Date()))
			.returning({ id: this.tables.invitations.id });
